...
set +e
timeout 120s qemu-system-x86_64     -M pc     -cpu max     -m 512M     -kernel "$kernel"     -append "rootwait root=/dev/vda console=tty1 console=ttyS0"     -drive "file=$rootfs,if=virtio,format=raw"     -netdev user,id=net0     -device virtio-net-pci,netdev=net0     -display none     -serial "file:$serial_log"     -no-reboot     -no-shutdown
qemu_status=$?
set -e
...
echo "B-nix Linux boot and network smoke test passed."
