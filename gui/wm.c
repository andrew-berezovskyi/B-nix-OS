...
// ==============================================================================
// 🔥 НОВЕ: КЕШУВАННЯ ФАЙЛІВ (Щоб не лагав жорсткий диск)
// ==============================================================================
...
static int fm_file_count = -1; // -1 означає "потрібно прочитати з диска"
...
static void vfs_create_named_file(const char* prefix, const char* ext, bool is_folder) {
    ...
    if (is_folder) { fs_mkdir(name); } 
    else { int fd = fs_open(name, O_CREAT | O_WRONLY); if (fd != -1) fs_close(fd); }
    
    fm_file_count = -1; // Кажемо провіднику оновити кеш при наступному малюванні!
}
...
void app_fm_draw(int x, int y, int w, int h) {
    ...
    // 🔥 ОПТИМІЗАЦІЯ: Використовуємо кеш замість диска
    if (fm_file_count == -1) refresh_fm_cache();

    for (int i = 0; i < fm_file_count; i++) {
        cached_file_t* entry = &fm_file_cache[i];
        if (custom_strcmp(selected_filename, entry->name) == 0) {
            draw_filled_rect(fx - 4, fy - 4, 84, 88, 0xD0E8FF);
        }
        
        if (entry->type == FS_TYPE_DIR) { draw_icon_folder(fx, fy); } 
        else { draw_icon_file(fx + 4, fy - 8); }
...
void app_fm_click(int mx, int my, bool right_click) {
    ...
    if (mx >= wx && mx < wx + left_w) {
        for (int i = 0; i < FM_NAV_COUNT; i++) {
            ...
            if (my >= ly - 4 && my < ly + 24) { fm_nav_sel = i; selected_filename[0] = '\0'; return; }
        }
        return;
    }
    ...
    // 🔥 ОПТИМІЗАЦІЯ: Кліки теж перевіряємо по кешу
    if (fm_file_count == -1) refresh_fm_cache();
    ...
    for (int i = 0; i < fm_file_count; i++) {
        ...
        if (mx >= fx - 4 && mx < fx + 84 && my >= fy - 4 && my < fy + 88) {
            ...
            if (custom_strcmp(selected_filename, entry->name) == 0 && entry->type == FS_TYPE_FILE) {
                ...
                custom_strcpy(selected_filename, entry->name);
            }
            break;
        }
        col++; if (col >= max_cols) { col = 0; fx = gx; fy += 100; } else fx += 92;
    ...
    if (!hit_file) selected_filename[0] = '\0';
}

// ==============================================================================
// ПРОГРАМА 3: ПЕРЕГЛЯДАЧ ФАЙЛІВ
// ==============================================================================
void app_viewer_draw(int x, int y, int w, int h) {
    if (main_font_data) {
        draw_ttf_string(x + 12, y + 16, main_font_data, viewer_content, 13.0f, 0xD0D0D0);
    }
}

// ==============================================================================
// WINDOW MANAGER (ДВИГУН)
// ==============================================================================
void wm_init(void) {
    ...
    focused_window = -1;
    fm_file_count = -1; // Примусове оновлення при старті
}
...
void wm_process_mouse(int mx, int my, bool left_now, bool right_now, bool j_c, bool j_r) {
    ...
    if (mx < DESKTOP_DOCK_W && my >= DESKTOP_TOP_BAR_H) {
        ...
        if (di == 0) { windows[0].is_open = true; focused_window = 0; }
        if (di == 1) { windows[1].is_open = true; focused_window = 1; }
        return;
    }
